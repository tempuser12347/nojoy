...
const columns = [
  ...
  { id: 'apartment_rank', label: '아파트 랭크', minWidth: 100 },
  { id: 'certificate', label: '증명서', minWidth: 150, format: (value: any) => value ? renderObjectChip(value, useNavigate()) : null },
];
...
const Pets: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pets', page, rowsPerPage, name_search],
    queryFn: async () => {
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>애완동물</Typography>
