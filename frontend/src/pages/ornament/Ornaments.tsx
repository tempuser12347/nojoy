...
const Ornaments: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get("/api/ornaments", {
        params: {
          name_search,
          sort_by,
          sort_order,
  ...
  const columns = [
    { id: "name", label: "이름", minWidth: 170, sortable: true },
    { id: "acquisition", label: "획득", minWidth: 100, sortable: true },
    { id: "crafter", label: "제작자", minWidth: 100, sortable: true },
    { id: "discovery_card", label: "발견물 카드", minWidth: 150, format: (value: { id: number, name: string }[]) => value ? renderObjectsToChips(value, navigate) : null },
    { id: "installation_effect", label: "설치 효과", minWidth: 200, format: (value: { id: number, name: string }[]) => value ? renderObjectsToChips(value, navigate, v=>'+'+v) : null },
    { id: "city", label: "도시", minWidth: 100, format: (value: { id: number, name: string }) => value ? renderObjectChip(value, navigate) : null },
  ];
