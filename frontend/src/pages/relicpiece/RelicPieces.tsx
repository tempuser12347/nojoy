...
const RelicPieces: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get("/api/relicpieces", {
        params: {
          name_search,
          sort_by,
          sort_order,
      ...
      const processedItems = response.data.items.map((item: any) => {
        return {
          ...item,
          theme_name: item.theme ? item.theme.name : "",
          quest_name: item.quest ? item.quest.name : "",
        };
      });
  ...
  const columns = [
    ...
    { id: "piece_rank", label: "피스 랭크", minWidth: 100 },
    { id: "theme_name", label: "테마", minWidth: 100 },
    { id: "quest_name", label: "퀘스트", minWidth: 100 },
  ];
