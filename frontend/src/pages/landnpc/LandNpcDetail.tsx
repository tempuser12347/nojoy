...
const FieldsTable: React.FC<{ data: { id: number; name: string }[] }> = ({ data }) => {
...
const TechniquesTable: React.FC<{ data: LandNpc['techniques'] }> = ({ data }) => {
    const navigate = useNavigate();
    if (!data || data.length === 0) return null;
                    ...
                    {Object.entries(groupedTechniques).map(([techType, techniques]) => {
                        ...
                        return techniques.map((tech, index) => (
                            <TableRow key={`${techType}-${index}`}>
                                ...
                                <TableCell>{renderObjectChip(tech['테크닉'], navigate)}</TableCell>
                            </TableRow>
                        ));
                    })}
                </TableBody>
...
const DropItemsTable: React.FC<{ data: LandNpc['drop_items'] }> = ({ data }) => {
    const navigate = useNavigate();
    if (!data || data.length === 0) return null;
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {npc.fields && npc.fields.length > 0 && (
        <Box mt={3}>
          <Typography variant="h5" gutterBottom>필드</Typography>
      ...
      {npc.techniques && npc.techniques.length > 0 && (
        <Box mt={3}>
          <Typography variant="h5" gutterBottom>테크닉</Typography>
      ...
      {npc.drop_items && npc.drop_items.length > 0 && (
        <Box mt={3}>
          <Typography variant="h5" gutterBottom>드롭 아이템</Typography>
