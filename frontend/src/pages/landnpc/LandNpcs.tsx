...
const columns = [
  ...
  { id: 'fields', label: '필드', minWidth: 200, format: (value: any) => value && value.map((f: any) => f.name).join(', ') },
];

const LandNpcs: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['landnpcs', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/landnpcs', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>육상 NPC</Typography>
