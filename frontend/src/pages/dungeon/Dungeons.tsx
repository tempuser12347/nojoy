...
const Dungeons: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get("/api/dungeons", {
        params: {
          name_search,
          sort_by,
          sort_order,
  ...
  const columns = [
    ...
    { id: "floors", label: "층수", minWidth: 100 },
    { id: "dungeon_rank", label: "랭크", minWidth: 80 },
    { id: "dungeon_exploration", label: "탐험", minWidth: 80 },
    { id: "boarding_pass", label: "승선권", minWidth: 80 },
    { id: "discoveries_objects", label: "발견물", minWidth: 200, format: (value: any[]) => renderObjectsToChips(value, navigate) },
  ];
