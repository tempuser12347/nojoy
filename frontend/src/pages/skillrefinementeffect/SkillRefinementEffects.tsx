...
const SkillRefinementEffects: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['skillrefinementeffects', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/skillrefinementeffects', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>스킬 연성 효과</Typography>
...
export default SkillRefinementEffects;
