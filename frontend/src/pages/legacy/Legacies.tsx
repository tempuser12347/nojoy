...
const Legacies: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get("/api/legacies", {
        params: {
          name_search,
          sort_by,
          sort_order,
  ...
  const columns = [
    ...
    { id: "theme", label: "테마", minWidth: 150, format: (value: any) => value ? renderObjectChip(value, navigate) : null },
    { id: "destination", label: "목적지", minWidth: 150, format: (value: any) => value ? renderObjectChip(value, navigate) : null },
    { id: "rewards_sophia", label: "소피아", minWidth: 80 },
    { id: "rewards_items", label: "보상 아이템", minWidth: 200, format: (value: any[]) => renderObjectsToChips(value, navigate) },
  ];
