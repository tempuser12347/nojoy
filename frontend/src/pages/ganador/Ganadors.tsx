...
const columns = [
  ...
  { id: 'durability', label: '내구력', minWidth: 100 },
  { id: 'crew', label: '선원', minWidth: 100 },
  { id: 'attack_power', label: '공격력', minWidth: 100 },
  { id: 'defense_power', label: '방어력', minWidth: 100 },
  { id: 'preparation_item', label: '준비물', minWidth: 200, format: (value: any) => value && `${value.name} x ${value.quantity}` },
];

const Ganadors: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['ganadors', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/ganadors', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>가나돌</Typography>
