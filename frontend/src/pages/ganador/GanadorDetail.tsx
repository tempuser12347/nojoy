...
const RequirementsTable: React.FC<{ data: Ganador['requirements'] }> = ({ data }) => {
...
const AcquiredItemsTable: React.FC<{ data: Ganador['acquired_items'] }> = ({ data }) => {
                    ...
                    {Object.entries(groupedItems).map(([type, items]) => {
                        ...
                        return items.map((item, index) => (
                            <TableRow key={`${type}-${item.id}`}>
                                {index === 0 && (
                                    <TableCell rowSpan={rowSpan}>{type}</TableCell>
                                )}
...
export default function GanadorDetail({ data }: { data?: Ganador }) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom>{ganador.name}</Typography>
      <Card>
        <CardContent>
      ...
      {ganador.requirements && ganador.requirements.length > 0 && (
        <Box mt={3}>
          <Typography variant="h5" gutterBottom>요구 사항</Typography>
