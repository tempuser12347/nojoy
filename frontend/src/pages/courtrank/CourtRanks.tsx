...
const columns = [
  ...
  { id: 'level', label: '레벨', minWidth: 100 },
  { id: 'ottoman', label: '오스만', minWidth: 100 },
  { id: 'royal_fleet_rank', label: '왕실 함대 랭크', minWidth: 100 },
  { id: 'fame', label: '명성', minWidth: 100 },
];

const CourtRanks: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['courtranks', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/courtranks', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>작위</Typography>
