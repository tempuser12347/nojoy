...
const ResearchActions: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['researchactions', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/researchactions', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>연구 행동</Typography>
