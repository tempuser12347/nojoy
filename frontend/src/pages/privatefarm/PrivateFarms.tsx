...
const columns = [
  { id: 'name', label: '농장명', minWidth: 170 },
  { id: 'description', label: '설명', minWidth: 170 },
];

const PrivateFarms: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['privatefarms', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/privatefarms', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>개인 농장</Typography>
