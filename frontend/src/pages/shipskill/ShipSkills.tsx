...
const columns = [
  ...
  { id: 'action_points', label: '행동력', minWidth: 100 },
  { id: 'apply_range', label: '적용범위', minWidth: 100 },
  { id: 'required_skill', label: '필요스킬', minWidth: 200, format: (value: any) => value && value.map((skill: any) => `${skill.name} ${skill.rank}`).join(', ') },
  { id: 'dedicated_skill', label: '전용함스킬', minWidth: 100, format: (value: any) => value ? '✅' : '❌' },
];

const ShipSkills: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['shipskills', page, rowsPerPage, name_search],
    queryFn: async () => {
      const response = await api.get('/api/shipskills', {
        params: {
          name_search,
  ...
  return (
    <Box sx={{ width: '100%', p: 3, height: 'calc(100vh - 100px)' }}>
      <Typography variant="h4" gutterBottom>선박 스킬</Typography>
